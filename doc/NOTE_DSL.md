...
**ANTLR解析器（ANother Tool for Language Recognition 另一种语言识别工具）(读音：昂特勒)是一个成熟的，广泛适用的开源工具，它是一个强大的递归下降语法解析器**
...
编写一个转换器来生成代码，它读入输入模型然后产生结果

### 52.1 工作原理

基于转换器的代码生成是这样一类程序，它读取语义模型作为输入，然后为目标环境生成源代码。通常把转换器分成两部分：输入驱动和输出驱动。输出驱动的转换从输出结果入手，然后按需要寻找对应的输入数据：而输入驱动的转换则整体读取输入数据结构，然后再生成输出。

通常转换器会组合使用这两种方式。很多转换直接从语义模型转换到目标代码，而对于复杂的转换过程，把它分解成多级是个有效的做法。比如，两级转换可以读入输入模型，产生输出模型而不是文本----虽然这个输出模型可能非常接近最终产生的文本：在第二级时，根据输出模型产生最终的文本。当转换器非常复杂或者需要从同一输入源（共享一些特性）产生多个输出文本时，使用多步转换是个好办法。因此可以在第一步时为常见元素生成单个输出模型，而把具体的文本输出差异交给后面的转换来完成。

当使用多级转换方法时，还可以组合使用多种技术，比如基于转换器的代码生成在第一级而**模版化的生成器**在第二级。

### 52.2 使用场景

当输出文本和输入模型有简单的映射关系且大部分输出文本可以自动产生时，单级基于转换器的代码生成是个不错的选择。在这种情况下，基于转换器的代码生成很容易编写而且不需要引入任何模版工具。

而当输入和输出结果之间存在复杂关系时，则可以使用多级基于转换器的代码生成，而每一级可以处理一个特定的代码生成中的方面。

当使用**基于模型的代码生成**时，通常可以使用基于转换器的代码生成来生成一些系列方法，然后依次调用它们来产生模型。

### 52.3 安全面板控制器（Java生成的C）

## 第53章 模版化的生成器

模版化的生成器的基本想法是编写希望的输出文件，然后插入标注(callout)以表示可变部分。然后使用具备模版文件模版处理器和相应语境，通过这个上下文填写标注进而生成真正的输出文件。

### 53.1 工作原理

大多数有动态内容的网站都会使用模版化的生成器。另外一个例子是C语言中的`printf`函数，它根据模版每次输出单个字符串。在代码生成的语境下，模版化的生成器通常都是针对整个文档的，不过`printf`提醒模版化的生成器和**基于转换器的代码生成**区别并不是那么大。文本宏处理器是软件开发中另一种形式古老的模版化的生成器。

模版化的生成器中有三个主要组件：模版引擎、模版和语境。模版是文本文件的源文本，其中动态部分由标注表示。在代码生成的过程中，标注则会调用语境对象来生成动态元素。语境则是动态数据的源----本质上说是生成模版所需的数据模型。语境可以是简单的数据结构，也可以是更复杂的可编程环境：不同的模版工具使用不同形式的语境对象。模版引擎是使用模版和语境来生成最终结果的工具，它控制模版生成程序的执行，结合模版和语境对象生成最终的结果。它也可以对不同的语境使用同样的模版，从而生成多个输出结果。

绝大多数模版处理器允许将任意宿主语言代码表达式放入模版标注中，像ASP或者JSP这样的工具就是这么做的。

因为模版文件很容易被过多的宿主代码搞砸，很多模版处理器不允许在标注中放置宿主语言代码，而是使用特定的模版语言。模版语言通常都会限制语法特性，以保证标注内容简洁和模版结构清晰。最简单的模版语言把映射作为语境对象，提供表达式从映射中查找值，并将值作为结果插入到输出中。对于简单的模版这种方法很有效，但大部分情况下这可能还不够。

在需要复杂模版技术的场景中，最常见的驱动因素就是根据一组对象产生结果的需求，它通常需要某种循环结构。另一个常见的需求是条件式生成，根据语境对象中不同的值需要不同的模版输出。

### 53.2 使用场景

模版化的生成器最大的好处在于，通过阅读模版文件，就可以很容易了解最终输出的样子。特别是当输出包含大量的静态内容，而仅仅少量简单的动态内容的时候，这非常有用。

需要使用模版化的生成器的第一个迹象是，在生成的文件中有大量的静态内容。静态内容越多，使用模版化的生成器就会越容易。此外还需要考虑待生成的动态内容的复杂度。所使用的循环、条件分支、高级模版语言特性越多，就越难从模版文件中看出最后生成的结果。如果必须使用这些复杂的特性，那么考虑使用**基于转换器的代码生成**。

### 53.3 生成带有嵌套条件的安全控制面板状态机（Velocity和Java生成的C）

<!-- TODO：待补充-->

## 第54章 嵌入助手

为了简化模版系统中包含的代码，某个对象提供了该模版机制所需的所有函数。

为了达到扩展能力的目的，许多系统允许在某个简单展现形式内嵌入通用代码，从而完成仅通过该形式无法完成的功能。比如，在Web页面模版内嵌入代码，在文法文件中放入行为代码，在代码生成模版中放入标注。

嵌入助手将所有复杂的代码都放入助手类中，而在宿主表示形式中仅仅保持简单的方法调用。这样可以保持宿主表示形式的简洁同时也能保持其主导地位。

### 54.1 工作原理

嵌入助手背后的思想很像是一种重构手法。创建嵌入助手使其对宿主表示形式可见，然后把宿主表示格式中的所有代码都搬到嵌入助手中，仅仅留下对嵌入助手的方法调用。

有一个潜在的棘手的技术问题：如何使用这个对象在处理宿主表示形式时在作用域中可见。绝大多数系统都提供了某种机制来完成它，但是有时候它可能会变得很混乱。

一旦使得嵌入助手可见，那么所有不是简单方法调用的代码都应该放入嵌入助手中，然后在宿主表示形式中留下的代码应该仅仅是简单的调用。

嵌入助手通常会结合**模版化的生成器**一起使用，在这种情况下，通常都会问这样一个问题：嵌入助手是否应该产生输出需要权衡实现复杂度，尽量避免从嵌入助手生成最终输出。

### 54.2 使用场景

当不采用嵌入助手的时候，非常难看出宿主表示方式中的内容，比如，在语法文件中使用大量外加代码的话，就非常难看清语法的基本结构。

使用嵌入助手的首要原因是保持宿主表示方式的清晰，此外还有一个好处体现在工具上，尤其是当使用成熟的IDE时更加明显。在这种情况下，嵌入代码无法直接编辑，而一旦把它们放入嵌入助手中，就可以利用IDE的编辑环境来修改代码了。

当使用的类很自然地提供了这些信息的时候，不需要使用嵌入助手。比如，当结合**语义模型**使用**模版化的生成器**时，这时嵌入助手的行为都很自然地属于语义模型。这时候就不需要引入额外的嵌入助手。
...
在生成的代码中隐式地模拟DSL的语义模型，从而实现了普遍性与特殊性的分离。当生成代码时，在代码中包含了DSL脚本的语义。通过使用基于模型的代码生成，可以在生成的代码中再现了某种形式的**语义模型**，从而可在其中保证普遍性与特殊性的分离。

### 55.1 工作原理

基于模型的代码生成最重要的特点就是它遵守了普遍性与特殊性分离的原则。生成的代码中的模型采用何种形式并不重要，因为其中包含了**语义模型**的**影模型(simulacrum model)**

将这个模型成为语义模型的影模型有很多原因。通常当生成代码时，目标环境都具有这样或那样的限制，这些限制使语义模型的表示通常不能如你所愿，因此必须做出相应的妥协和退让，这会使得语义模型无法有效地表示系统的意图。

因为模拟模型是语义模型的独立版本，所以影模型可以并应该在不使用代码生成的前提下构建并测试。应该为这个模型提供一个简单的构造API。这样，代码生成器就可以通过使用这个API来生成配置代码，还可以通过这个API来生成配置代码，还可以通过这个API来编写测试脚本对模拟模型进行测试。这样可以通过运行代码生成过程构造、测试以及细化目标环境的核心行为。通过简单的测试来构造模型，也简化了调试和理解模型的难度。

### 55.2 使用场景

比起**无视模型的代码生成**，基于模型的代码生成具有很多优势。不依赖于代码生成的影模型可以更容易构造和测试，因为不需要再次运行和理解代码的生成过程就可以使用和测试它。同时，由于生成的代码主要由影模型的API组成，这样的代码更容易生成，同时代码生成器也更容易编写和维护。

不实用基于模型的代码生成的主要原因是因为目标环境的限制，可能无法有效地表达模型----甚至是影模型，也可能因为性能方面的考虑，无法在运行时使用影模型。

在很多情况下，可以把DSL当做某个已经存在的模型的前端来使用。如果是通过代码生成来使用这个模型，就是在使用基于模型的代码生成。
...
将所有逻辑都硬编码到生成的代码中，以便没有**语义模型**的显式表示方式。

### 56.1 工作原理

代码生成的一个优点是，它允许在一个受控的环境下生成代码，而无须手工重复编写。在实现层面上，这种方法通常会引入重复代码，特别是在使用某种数据结构表达式行为而后再把它编码到控制流中的情况下。

当使用无视模型的代码生成时，可以从目标环境中为一段特定的DSL脚本提供实现开始。实现代码要尽量写的清晰，然而却不必太过在意是否区分了特殊和通用的代码。对于代码中特定元素的重复也不必计较，因为最终这部分都将由代码生成器来生成。

### 56.2 使用场景

有时候目标环境中所使用的语言，对于代码结构和模型的描绘仅仅提供有限的支持。在这种情况下，就无法使用**基于模型的代码生成**了，此时，无视模型的代码生成成了唯一的选择。另外一种情况是，当使用基于模型的代码生成时，得到的结果需要太多的运行时资源，可以把无视模型的代码生成作为一种优化代码的手段来使用。将逻辑直接编码到控制流中，能够减少对于内存的需求，也可能提高系统的性能：如果内存使用情况和性能两者都是非常重要的，那么无视模型的代码生成也是一种不错的方式。

然而，总体而言，倾向于基于模型的代码生成。通常而言，由基于模型的代码生成产生代码更容易，在生成的程序中理解和修改。而无视模型的代码生成通常生成的代码虽然在结构上更容易搞清楚代码到底做了什么，但是其中概念没有那么清晰。
...
代沟通过继承分离生成的代码和非生成的代码。

### 57.1 工作原理

**代沟(Generation Gap)**的基本形式是，生成父类----Vlissides称为核心类----然后手动编写子类。这样就可以始终在子类中任意地重写生成的代码。手写代码可以轻松调用任何生成的动能，而生成的代码也可以通过抽象方法调用手动编写的动能，而编译器能检查抽象方法由子类实现，或者由仅根据需要重写的钩子方法实现。

另外一种方式是，手写的类作为生成的类的超类，这个类包含所有与生成的代码无关的逻辑。比起自动生成这些无变化的代码，将它们放在超类中，工具可以更容易地追踪它们。一般而言，对于代码生成的建议是，尽可能少地生成代码，这是因为生成的代码比起手写的代码更难修改。

继承结构中可能存在三个类：

* 手写的基类，其所含逻辑不随代码生成器的输入参数变化
* 生成的类，其所含逻辑由代码生成器自动生成。
* 手写的具体类，其所含逻辑无法自动生成，也不依赖生成的特性。只有这个类可以被其他代码调用。

并不总是需要全部的类，如果没有不变的逻辑，就不需要手动编写基类：同样，如果不需要重写生成的代码，也可以跳过手写的具体类----代沟另一个可行的变体：手写的超类和生成的子类。

通常会发现手写类和生成类之间的结构会变得很复杂，复杂的继承，复杂的调用。手写代码与生成代码之间的交互确实导致更复杂的类结构，这是使用代码生成器的便利付出的代价。

随代沟出现的皱纹是：当某些时候而非所有时刻拥有手写的具体类时，应该怎么办的问题。在这种情况下，需要决定如何处理那些不需要手动编写具体类的情况。可以调整那些类的名字，让它们和手写的具体类具有相同的命名规范，但是这样会带来不必要的命名和用途困惑。因此倾向于创建空白的具体类，但是不让它们重写任何方法。

### 57.2 使用场景

代沟是一种很有效的技术，它允许你将某个逻辑类分隔为多个文件，从而保持生成的代码的独立性。不过所用的语言需要支持继承才能使用它。使用继承意味着可以重写父类中的方法，这需要有足够的访问权限。比如`private`关键字就不是很好的选择。

如果所使用的语言允许将同一个类的代码放置在不同的文件里，那么这种特性也可以达到代沟的效果。

代沟不是唯一能使生成的代码和手写的代码分离的方式。将生成的代码放入一个类而手写的代码放入另一个类，然后让它们彼此调用也是可行的方式。

### 57.3 根据数据结构生成类（Java和Ruby）
