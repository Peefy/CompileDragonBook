...
尽管闭包作为一种语言特性已经存在很久了，但直到最近它才被很多软件开发人员所关注。闭包是可以当做对象的代码块。本质上，闭包是一种更优雅的方案，可以很容易地创建一个代码段，然后把它作为对象传递，不同的语言中对闭包的定义有所不同，比如C+和Python中使用lambda表达式，C#中使用lambda表达式和匿名函数。
